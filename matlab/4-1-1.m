function impulse_response
    ...
    t_span = [0, 10];
    t_values = linspace(t_span(1), t_span(2), 2000);
    q_values = zeros(N, length(t_values));

    for i = 1:N
        initial_conditions = [0, 0];
        [t, sol] = ode45(@(t, q) odefun_impulse(t, q, a(i), b(i), c(i), w), t_span, initial_conditions);
...
function dydt = odefun_impulse(t, q, a, b, c, w)
    % Approximation of unit impulse function
    if abs(t) < 1e-3 % Adjust this threshold for impulse width
        A = 1e9; % Adjust this for impulse amplitude
    else
        A = 0;
    end
