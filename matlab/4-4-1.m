function quadruple_impulse_response
    % Constants
    ...
    for i = 1:N
        ...
        [t, sol] = ode45(@(t, q) odefun_quadruple_impulse(t, q, a(i), b(i), c(i), w), t_span, initial_conditions);
...
function dydt = odefun_quadruple_impulse(t, q, a, b, c, w)
    % Approximation of quadruple impulse function (r = delta(t)^4)
    if abs(t) < 1e-12 % Extremely narrow width
        A = 1e18; % Extremely high amplitude
    else
        A = 0;
    end
...
function plot_results(t_values, q_values, x_range, y_values_2d, N)
    ...
    subplot(3, 3, N + 1);
    selected_time points = [10, 20, 30, 40, 50];
