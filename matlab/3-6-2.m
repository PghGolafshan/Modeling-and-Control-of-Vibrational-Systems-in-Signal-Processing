function five_sine
   ...
   t_values = linspace(t_span(1), t_span(2), 100);

   % Solve for each q_i(t)
   q_values = zeros(N, length(t_values));
   for i = 1:N
       initial_conditions = [0, 0];  % Initial conditions set to zero
       [t, sol] = ode45(@(t, q) odefun(t, q, a(i), b(i), c(i), w(i)), t_span, initial_conditions);
...
function dydt = odefun(t, q, a, b, c, w)
   A = sin(w * t);  % Sinusoidal input with frequency w
