function main
   ...
   % Coefficients for each mode (adjust these based on your system)
   ...
   for i = 1:N
       ...
       [t, sol] = ode45(@(t, q) odefun(t, q, a(i), b(i), c(i), w, Delta), t_span, initial_conditions);
...
function dydt = odefun(t, q, a, b, c, w, Delta)
   % Delta function approximation as a narrow rectangular pulse
   A = delta_approx(t, Delta);
   dydt = [q(2); A * c - a * q(2) - b * q(1)];
end

function y = delta_approx(t, Delta)
   % This function approximates the delta function as a narrow rectangular pulse
   y = (abs(t) <= Delta) .* (1 / (2 * Delta));
end
...
% Set Delta for the approximation of the delta function
Delta = 1e-3;  % For example, a very small value
