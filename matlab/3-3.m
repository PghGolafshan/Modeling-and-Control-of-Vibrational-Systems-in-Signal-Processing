function sine
   ...
   for i = 1:N
       initial_conditions = [0.1 / i, 0];  % q_i(0) = 0.1/i, q_i'(0) = 0
       [t, sol] = ode45(@(t, q) odefun(t, q, a(i), b(i), c(i)), t_span, initial_conditions);
       q_values(i, :) = interp1(t, sol(:, 1), t_values);
   end
   % Range of x values
   x_range = linspace(0, 0.25, 100);
   ...
   subplot(3, 3, N + 1);
   selected_time_points = [0, 0.5, 1, 1.5, 2];  % Example time points in seconds
...
function dydt = odefun(t, q, a, b, c)
   A = sin(t);  % Changed to a sinusoidal input
